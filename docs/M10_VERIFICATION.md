...
| poison_damaged 信号 | tick 时 emit poison_damaged | MCP: 连接后触发确认 | [ ] |
| healed 信号 | heal 时 emit healed | MCP: 连接后触发确认 | [ ] |
...
| tick 杀死玩家 | poison tick 致死 → combat_ended(defeat) | MCP: hp=2, poison=3 → defeat | [ ] |
| tick 杀死敌人 | 敌人 poison tick 致死 → combat_ended(victory) | MCP: enemy.hp=2, poison=3 → victory | [ ] |

### 0.3 状态显示

| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| poison 状态文本 | _format_status_text 显示 ☠️N | MCP: entity.add_poison(3) → label 包含 ☠ | [ ] |
| thorns 状态文本 | _format_status_text 显示 🌵N | MCP: entity.add_thorns(3) → label 包含 🌵 | [ ] |
| poison 浮动文字 | poison tick 时显示绿色 ☠️-N | 手动 | [ ] |
| heal 浮动文字 | heal 时显示绿色 +N | 手动 | [ ] |
...
| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| damage 执行 | intent.damage > 0 → player.take_damage | MCP: 设置 intent 并执行 | [ ] |
| block 执行 | intent.block > 0 → enemy.gain_block | MCP: intent.block=5 → enemy.block=5 | [ ] |
| strength 执行 | intent.strength > 0 → enemy.add_strength | MCP: intent.strength=2 → enemy.strength=2 | [ ] |
| poison 执行 | intent.poison > 0 → player.add_poison | MCP: intent.poison=4 → player.poison=4 | [ ] |
| weak 执行 | intent.weak > 0 → player.add_weak | MCP: intent.weak=1 → player.weak=1 | [ ] |
| hits 多段 | hits=2 → 执行 2 次 damage | MCP: hits=2, damage=6 → player 受 2 次 | [ ] |
| thorns 反伤 | enemy 攻击时 player 有 thorns → enemy 受伤 | MCP: player.thorns=3, enemy攻击 → enemy受伤 | [ ] |
...
| 视觉：红色矩形 | 红色 Color(0.5,0.1,0.1) 150×120 | MCP: enemy_visual 颜色/大小确认 | [ ] |
...
| Spore Cloud 施加 poison | 执行时 player.poison += 4 | MCP: 选到 Spore Cloud 后 player.poison=4 | [ ] |
...
| Rake 施加 weak | 执行时 player.weak += 1 | MCP: 选到 Rake 后 player.weak=1 | [ ] |
| 视觉：灰紫色 | Color(0.35,0.15,0.45) 150×120 | MCP: 确认 | [ ] |
...
| 视觉：深红+金边 | 深红+金色边框 170×130 | MCP: 确认 | [ ] |
...
| 阶段切换 | HP ≤ 50% → 切换阶段 2 招式表 | MCP: hp=39/80 → phase 2 | [ ] |
| 阶段 2 招式 | Multi-Strike(6×2)/Blood Rage(12,+3str)/Harden(block=20) | MCP: dmg=6,hits=2确认 | [x] |
| Multi-Strike 命中 | hits=2 → player 受 2 次 6 伤害 | MCP: intent hits=2 执行后 hp-12 | [ ] |
| 视觉：暗红+大尺寸 | Color(0.45,0.05,0.15) 200×160 3px边框 | MCP: 确认 | [ ] |
| 视觉：boss 边框加粗 | border_width = 3（vs 普通 2） | MCP: 确认 | [ ] |
...
| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| poison:N 效果 | 出牌 → enemy.add_poison(N) | MCP: 模拟 Poison Stab → enemy.poison=3 | [ ] |
| weak:N 效果 | 出牌 → enemy.add_weak(N) | MCP: 模拟 Crippling Blow → enemy.weak=2 | [ ] |
| heal:N 效果 | 出牌 → player.heal(N) | MCP: 模拟 Bandage → player.hp 恢复 | [ ] |
| thorns:N 效果 | 出牌 → player.add_thorns(N) | MCP: 模拟 Thorns → player.thorns=3 | [ ] |
| shield_bash 效果 | 出牌 → damage = player.block | MCP: block=8 → damage=8 | [ ] |
| thorns 反伤 | player 有 thorns，敌人攻击 → 敌人受 thorns 伤害 | MCP: thorns=3, 敌人攻击 → enemy.hp-3 | [ ] |

### 3.3 奖励池

| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| 奖励池大小 | 13 张非 Starter 卡（7 旧 + 6 新） | MCP: reward_pool size=13 | [ ] |
| 新卡可被选中 | 奖励 3 选 1 中可出现新卡 | MCP: 多次 _pick_3_rewards 验证 | [ ] |
| 新卡可实例化 | cfc.instance_card("Poison Stab") 不报错 | MCP: instance 确认 | [ ] |
...
| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| 敌人不同 | 第 1 场和第 2 场敌人名称不同 | MCP: encounter 1 name ≠ encounter 2 name | [ ] |
| Boss 第 3 场 | 第 3 场敌人名 "Heart Mimic" | MCP: encounter 3 name=Heart Mimic | [ ] |
| advance_encounter | 正确推进到下一场 | MCP: advance → encounter+1 | [ ] |
...
| 背景有渐变纹理 | GradientTexture2D 不为空 | MCP: texture != null | [ ] |
| 普通场背景色 | 深蓝渐变 | 手动 | [ ] |
| 精英场背景色 | 暗紫渐变 | 手动 | [ ] |
| Boss 场背景色 | 暗红渐变 | 手动 | [ ] |
| z_index | 背景在所有 UI 之下 | MCP: z_index=-10 | [ ] |

### 5.2 敌人受击反馈

| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| 闪白效果 | 敌人受击时 modulate 变白再恢复 | 手动 | [ ] |
| 抖动效果 | 敌人受击时 position 随机偏移再恢复 | 手动 | [ ] |
...
| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| Jaw Worm 视觉 | 红色 150×120 | MCP: 遇到时确认 | [ ] |
| Fungi Beast 视觉 | 绿色 120×100 近圆形 | MCP: 遇到时确认 | [ ] |
| Slaver 视觉 | 灰紫色 150×120 | MCP: 遇到时确认 | [ ] |
| Elite 视觉 | 深红+金边 170×130 | MCP: 第 2 场确认 | [ ] |
| Boss 视觉 | 暗红 200×160 3px 边框 | MCP: 第 3 场确认 | [ ] |
| 动态定位 | HP 条/状态标签根据敌人大小定位 | MCP: HP bar 在 enemy_visual 下方 | [ ] |

### 5.5 卡牌背景差异化（#8 验证）

| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| Attack 卡面红色 | Strike/Bash 等使用 AttackFront (0.55,0.06,0.06) | MCP: instance Strike 确认 card_front | [ ] |
| Skill 卡面绿色 | Defend 等使用 SkillFront (0.06,0.3,0.12) | MCP: instance Defend 确认 card_front | [ ] |
| Power 卡面紫色 | Inflame 等使用 PowerFront (0.22,0.06,0.38) | MCP: instance Inflame 确认 card_front | [ ] |
...
| 信号连接 | poison_damaged × 2, healed × 1, thorns_triggered | MCP: 信号连接确认 | [ ] |
| EnemyDatabase 引用 | RunState 可以正确加载敌人配置 | MCP: encounter非空确认 | [x] |
| 3 场战斗无累积延迟 | 连续完成 3 场无卡顿 | 手动 | [ ] |
...
| 检查项 | 预期表现 | 验证 | 通过 |
|--------|----------|------|----|
| MainMenu → 战斗 | 正常进入第 1 场 | 手动 | [ ] |
| 拖拽出牌 | M8 拖拽功能不受影响 | 手动 | [ ] |
| 出牌闪光 | M8 出牌动画不受影响 | 手动 | [ ] |
| HP 条动画 | M8 HP 平滑过渡不受影响 | 手动 | [ ] |
| 奖励界面 | M9 奖励 3 选 1 正常工作 | 手动 | [ ] |
| 完整 3 场战斗 | 第 1 场随机 → 第 2 场精英 → 第 3 场 Boss → Run Complete | 手动 | [ ] |
| Game Over | 玩家死亡 → Game Over → 返回主菜单 | 手动 | [ ] |
...
- [ ] 不同敌人颜色/形状区分明显（红 Jaw Worm / 绿 Fungi Beast / 紫灰 Slaver / 金边 Elite / 大暗红 Boss）
- [ ] Boss 比普通敌人明显更大
- [ ] 普通场深蓝背景 vs 精英场暗紫 vs Boss 场暗红，色调差异可感知
- [ ] 敌人受击闪白+抖动反馈自然（不过于剧烈）
- [ ] 玩家受击屏幕边缘闪红反馈明显但不过于刺眼
- [ ] Poison tick 绿色 ☠️ 浮动文字清晰可辨
- [ ] Heal 绿色 +N 浮动文字清晰
- [ ] Thorns 反伤在敌人身上显示伤害数字
- [ ] Boss 阶段 2 Multi-Strike 连续命中时有视觉反馈
- [ ] 新卡牌卡面正确显示（Attack 红/Skill 绿/Power 紫）
